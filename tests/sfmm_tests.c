...
// Test to ensure payload is not messed with when allocating the next block
Test(sfmm_student_suite, student_test_1, .timeout = TEST_TIMEOUT) {
	unsigned int* i = sf_malloc(sizeof(unsigned int));
	unsigned int comp = -1;
	*i = comp;

	void* x = sf_malloc(sizeof(int));
	cr_assert_not_null(x, "x is NULL!");
	cr_assert_eq(*i, comp, "Payload field of block is being tampered with!");
}

// Test functionality of the bytesToFreeListIndex function
Test(sfmm_student_suite, student_test_2, .timeout = TEST_TIMEOUT){
	cr_assert_eq(bytesToFreeListIndex(32), 0, "32 bytes is not free list 0!");
	cr_assert_eq(bytesToFreeListIndex(64), 1, "64 bytes is not free list 1!");
	cr_assert_eq(bytesToFreeListIndex(128), 2, "128 bytes is not free list 2!");
	cr_assert_eq(bytesToFreeListIndex(256), 3, "256 bytes is not free list 3!");
	cr_assert_eq(bytesToFreeListIndex(512), 4, "512 bytes is not free list 4!");
	cr_assert_eq(bytesToFreeListIndex(1024), 5, "1024 bytes is not free list 5!");
	cr_assert_eq(bytesToFreeListIndex(2048), 6, "2048 bytes is not free list 6!");
	cr_assert_eq(bytesToFreeListIndex(4096), 7, "4096 bytes is not free list 7!");
	cr_assert_eq(bytesToFreeListIndex(8192), 8, "8192 bytes is not free list 8!");
	cr_assert_eq(bytesToFreeListIndex(16384), 9, "16384 bytes is not free list 9!");
	cr_assert_eq(bytesToFreeListIndex(32768), 9, "32768 bytes is not free list 9!");
}

// Test to ensure coalescing three blocks works properly (current test is only 2)
Test(sfmm_student_suite, student_test_3, .timeout = TEST_TIMEOUT){
	size_t sz_w = 200, sz_x = 200, sz_y = 300;
	void *w =  sf_malloc(sz_w);
	void *x = sf_malloc(sz_x);
	void *y = sf_malloc(sz_y);
...
// Test functionality of the NEXT_BLOCK macro
Test(sfmm_student_suite, student_test_4, .timeout = TEST_TIMEOUT){
	...
	sf_block* xb = (sf_block*)(x - 2*sizeof(sf_header));
	sf_block* yb = (sf_block*)(y - 2*sizeof(sf_header));

	cr_assert(NEXT_BLOCK_D(xb) == yb, "NEXT_BLOCK not returning the proper next block!");
}

// Test that allocated blocks are of the proper effective size (16 aligned) and GET_SIZE macro
Test(sfmm_student_suite, student_test_5, .timeout = TEST_TIMEOUT){
	...
	cr_assert(GET_SIZE_D(xb->header) == 32, "Block not pulled up to minimum block size!");
	cr_assert(GET_SIZE_D(yb->header) == 112, "Block size not a multiple of 16!");
}
